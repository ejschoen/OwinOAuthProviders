...
{
    public class VenmoAuthenticationHandler : AuthenticationHandler<VenmoAuthenticationOptions>
    {
        ...
        public VenmoAuthenticationHandler(HttpClient httpClient, ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                JObject user = JObject.FromObject(response.user);
                //// Get the Venmo user using the user info endpoint, which is part of the token - response.id
                ...
                var context = new VenmoAuthenticatedContext(Context, user, accessToken, refreshToken);
                ...
                {
                    context.Identity.AddClaim(new Claim(ClaimTypes.MobilePhone, context.Phone, XmlSchemaString, Options.AuthenticationType));
                }
        ...
        private async Task<bool> InvokeReplyPathAsync()
        {
            if (Options.CallbackPath.HasValue && Options.CallbackPath == Request.Path)
            {
                ...
                var context = new VenmoReturnEndpointContext(Context, ticket)
                    {
